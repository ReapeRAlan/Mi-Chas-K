"""
Adaptador de Base de Datos Mejorado - Versión 4.0 FINAL
...
class DatabaseAdapter:
        ...
        try:
            with sqlite3.connect(self.local_db_path) as local_conn:
                ...
                with render_conn.cursor() as remote_cursor:
                    # Reordenar items por prioridad de foreign keys
                    priority_order = {'categorias': 1, 'vendedores': 2, 'productos': 3, 'ventas': 4, 'detalle_ventas': 5}
                    sorted_items = sorted(pending_items, key=lambda x: (
                        priority_order.get(x[1], 6),  # tabla
                        0 if x[2] == 'INSERT' else 1 if x[2] == 'UPDATE' else 2  # operación
                    ))
                    ...
                    for item in sorted_items:
                        try:
                            ...
                            # Rollback en PostgreSQL para limpiar transacción
                            try:
    ...
    def _sync_insert_robust(self, cursor, table_name: str, data: Dict) -> bool:
        ...
        try:
            ...
            # Filtrar campos de metadata
            metadata_fields = ['original_query', 'original_params', 'timestamp', 'metadata', 'tags']
            clean_data = {k: v for k, v in adapted_data.items() if k not in metadata_fields}
            
            if not clean_data:
                logger.warning(f"⚠️ No hay datos válidos para insertar en {adapted_table}")
                return True
            
            # Construir query de inserción con validación de parámetros
            ...
            if len(values) != len(placeholders):
                logger.error(f"❌ Discrepancia de parámetros: {len(values)} valores, {len(placeholders)} placeholders")
                return False
    ...
    def _sync_update_robust(self, cursor, table_name: str, data: Dict) -> bool:
        ...
        try:
            ...
            # Verificar que el número de parámetros coincida
            if len(values) != len(set_clauses) + 1:  # +1 para el WHERE id
                logger.error(f"❌ Discrepancia de parámetros UPDATE: {len(values)} valores, {len(set_clauses)+1} esperados")
                return False
